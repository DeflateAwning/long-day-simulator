...
default_sim_config = SimConfig(
    ...
    # Note: 28h allows making it to one weekly meeting at the same awake time each week.
...
def run_sim_print_data(sim_config: SimConfig):
    ...
    cur_dt: datetime = datetime.combine(
        sim_config.start_day, sim_config.bedtime_on_start_day
    )
    print(f"Go to sleep at {cur_dt}")

    _last_awake_start: datetime = cur_dt - sim_config.awake_duration
    ...
    while cur_dt <= sim_config.stop_date_time:
        ...
        _last_awake_start = next_awake_start
    ...
    write_to_ics_files(sleep_periods, awake_periods, {"sleep"}, "long_day_schedule.ics")

    print(
        f"Wrote {len(sleep_periods)} sleep periods and {len(awake_periods)} awake periods to file."
    )
